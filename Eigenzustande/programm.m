...
Anzahl=[1,2,3,4,5,6,7,8]      #Anzahl der Perioden
  ...
  #function H_f=H_F(H_0,E,phi,r1,r2,Anzahl)
    for l = 1:length(Frequenz)
        for k = 1:length(Energien)
          ...
          save(['build/epsilon_fur_a=' num2str(Potential(i)*100) '_E=' num2str(Energien(k)*100) '_w=' num2str(Frequenz(l)*1000) 'a_N=' num2str(Anzahl(j) ) '.txt'],'epsilon')
          save(['build/Realpart_Eigenzustande_fur_a=' num2str(Potential(i)*100) '_E=' num2str(Energien(k)*100) '_w=' num2str(Frequenz(l)*1000) 'a_N=' num2str(Anzahl(j) ) '.txt'],'real_phi')
          save(['build/Imagpart_Eigenzustande_fur_a=' num2str(Potential(i)*100) '_E=' num2str(Energien(k)*100) '_w=' num2str(Frequenz(l)*1000) 'a_N=' num2str(Anzahl(j) ) '.txt'],'imag_phi')
