...
for a in tqdm(range(int(np.size(Potential)))):
    ...
    for e in tqdm(range(int(np.size(Energien)))):
        ...
        for l in tqdm(range(np.size(Anzahl_N))):
            ...
            #plt.title(r'Orthogonalitat für n=' + str(Anzahl_N[l]) + 'E=' +str(Energien[e]/10000) + ' a=' +str(Potential[a]/10000))
            plt.plot(Frequenz, werte_1[0,:].real,'-g',alpha=0.5,label=r'$\langle\Phi_1|\Phi_1\rangle$' )
            plt.plot(Frequenz, werte_1[1,:].real,'-b',alpha=0.5,label=r'$\langle\Phi_1|\Phi_2\rangle$' )
            plt.plot(Frequenz, werte_1[2,:].real,'-y',alpha=0.5,label=r'$\langle\Phi_1|\Phi_3\rangle$' )
            plt.plot(Frequenz, werte_1[3,:].real,'-r',alpha=0.5,label=r'$\langle\Phi_1|\Phi_4\rangle$' )
            plt.ylim(-1.5,1.5)
            plt.legend(loc='best')
