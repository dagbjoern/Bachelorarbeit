...
for a in tqdm(range(int(np.size(Potential)))):
    ...
    for e in tqdm(range(int(np.size(Energien)))):
        ...
        for l in tqdm(range(np.size(Anzahl_N))):
            ...
            plt.plot(Frequenz, werte_1[2,:].real,'-r',alpha=0.5,label=r'$\langle\Phi_1|\Phi_3\rangle$' )
            plt.plot(Frequenz, werte_1[3,:].real,'-y',alpha=0.5,label=r'$\langle\Phi_1|\Phi_4\rangle$' )
            plt.ylim(-1.5,1.5)
            plt.legend(loc='best')
