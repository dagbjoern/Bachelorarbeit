...
for a in tqdm(range(int(np.size(Potential)))):
    ...
    for e in tqdm(range(int(np.size(Energien)))):
        ...
        for l in tqdm(range(np.size(Anzahl_N))):
            ...
            plt.plot(Frequenz, werte_1[0,:].real,'-y',alpha=0.5,label=r'$\langle\Phi_1|\Phi_1\rangle$' )
            plt.plot(Frequenz, werte_1[1,:].real,'-r',alpha=0.5,label=r'$\langle\Phi_1|\Phi_2\rangle$' )
            plt.plot(Frequenz, werte_1[2,:].real,'-g',alpha=0.5,label=r'$\langle\Phi_1|\Phi_3\rangle$' )
