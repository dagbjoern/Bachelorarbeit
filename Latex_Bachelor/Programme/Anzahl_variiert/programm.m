...
Anzahl=[0,1,2,3,4,5,6,7]      #Anzahl der Perioden
...
#function H_f=H_F(H_0,E,phi,r1,r2,Anzahl)
    for l = 1:length(Frequenz)
        for k = 1:length(Energien)
          ...
          save(['build/Eigenwerte_fur_a=' num2str(Potential(i)*100) '_w=' num2str(Frequenz(l)*1000) '_E=' num2str(Energien(k)*100) '_N=' num2str(Anzahl(j)) '.txt'],'e')
