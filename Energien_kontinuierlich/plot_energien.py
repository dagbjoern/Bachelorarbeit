...
# plt.title('Eigenwerte von a='+str(value_Potenial/10000)+' w='+str(value_Frequenz)+' N='+str(Anzahl))
# plt.plot(x,x*0+value_Frequenz/2,'--k',alpha=0.5)
# plt.legend(loc='best')
# plt.xlabel('E')
# plt.ylabel(r'$\epsilon_\alpha $')
# plt.savefig('Plots/Plot_fur'+'_a='+str(value_Potenial/10000)+'_w='+str(value_Frequenz)+'N='+str(Anzahl) +'.pdf')
...
for index_Potential , value_Potenial in enumerate(tqdm(Potential)):
    for index_Frequenz , value_Frequenz in enumerate(tqdm(Frequenz)):
        for index_N ,value_N in enumerate(tqdm(Anzahl_N)):
            ...
            Matrix_mit_Eigenwerten=Eigenwerte_Matrix(value_Potenial,value_Frequenz*10000,value_N)
#           Matrix_mit_Erwarteteneigenwerten=eig_erwartet_Matrix(Anzahl,x,value_Potenial)
            n, m=np.shape(Matrix_mit_Eigenwerten)
            plt.title('Eigenwerte von a='+str(value_Potenial/10000)+' w='+str(value_Frequenz)+' N='+str(value_N))
